...
void Simulator::drawUI(sf::RenderWindow &window) const
{
  ...
  if (showInputField && currentFont && inputFieldText)
  {
    ...
    sf::Text instructText(*currentFont);
    instructText.setString(
        "Enter expression (use . for AND, + for OR, ~ for NOT, ^ for "
...
void Simulator::generateTruthTable()
{
  ...
  std::cout << "Generated truth table with " << combinations << " rows"
            << std::endl;
}
...
void Simulator::generateExpressionTruthTable()
{
  ...
  std::cout << "Generated truth table for expression: " << expression
            << std::endl;
...
void Simulator::setupUITexts() const
{
  ...
  expressionText->setString(currentExpression.empty() ? "No expression"
                                                      : currentExpression);
  expressionText->setCharacterSize(14);
  expressionText->setFillColor(sf::Color::Yellow);
  std::cout << "setupUITexts: expressionText set to: "
            << (currentExpression.empty() ? "No expression" : currentExpression)
            << std::endl;
  ...
  std::cout << "setupUITexts: Initialized " << truthTableTexts.size()
            << " truth table rows" << std::endl;

  inputFieldText = std::make_unique<sf::Text>(*currentFont);
  inputFieldText->setString(inputExpression.empty() ? "Enter expression..."
                                                    : inputExpression);
